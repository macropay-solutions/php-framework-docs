---
title: Obvious Getting Started
description: Introduction to the Obvious ORM, models, and basic database interactions in PHP Framework.
context: obvious
---

# Obvious: Getting Started

- [Introduction](#introduction)
- [Generating Model Classes](#generating-model-classes)
- [Obvious Model Conventions](#obvious-model-conventions)
...
Kernel includes Obvious, an object-relational mapper (ORM) that makes it enjoyable to interact with your database. When using Obvious, each database table has a corresponding "Model" that is used to interact with that table. In addition to retrieving records from the database table, Obvious models allow you to insert, update, and delete records from the table as well.
...
To get started, let's create an Obvious model. Models typically live in the `app\Models` directory and extend the `MacropaySolutions\Kernel\Database\Obvious\Model` class. You may use the `make:model` [run command](/run) to generate a new model:

```shell
php run make:model Flight
```

If you would like to generate a [database migration](/migrations) when you generate the model, you may use the `--migration` or `-m` option:

```shell
php run make:model Flight --migration
```
...
php run make:model Flight --factory
php run make:model Flight -f

# Generate a model and a FlightSeeder class...
php run make:model Flight --seed
php run make:model Flight -s

# Generate a model and a FlightController class...
php run make:model Flight --controller
php run make:model Flight -c

# Generate a model, FlightController resource class, and form request classes...
php run make:model Flight --controller --resource --requests
php run make:model Flight -crR

# Generate a model and a FlightPolicy class...
php run make:model Flight --policy

# Generate a model and a migration, factory, seeder, and controller...
php run make:model Flight -mfsc

# Shortcut to generate a model, migration, factory, seeder, policy, controller, and form requests...
php run make:model Flight --all

# Generate a pivot model...
php run make:model Member --pivot
php run make:model Member -p
```

<a name="inspecting-models"></a>
#### Inspecting Models

Sometimes it can be difficult to determine all of a model's available attributes and relationships just by skimming its code. Instead, try the `model:show` run command, which provides a convenient overview of all the model's attributes and relations:

```shell
php run model:show Flight
```

<a name="obvious-model-conventions"></a>
## Obvious Model Conventions

Models generated by the `make:model` command will be placed in the `app/Models` directory. Let's examine a basic model class and discuss some of Obvious's key conventions:
...
After glancing at the example above, you may have noticed that we did not tell Obvious which database table corresponds to our `Flight` model. By convention, the "snake case", plural name of the class will be used as the table name unless another name is explicitly specified. So, in this case, Obvious will assume the `Flight` model stores records in the `flights` table, while an `AirTrafficController` model would store records in an `air_traffic_controllers` table.
...
Obvious will also assume that each model's corresponding database table has a primary key column named `id`. If necessary, you may define a protected `$primaryKey` property on your model to specify a different column that serves as your model's primary key:
...
In addition, Obvious assumes that the primary key is an incrementing integer value, which means that Obvious will automatically cast the primary key to an integer. If you wish to use a non-incrementing or a non-numeric primary key you must define a public `$incrementing` property on your model that is set to `false`:
...
Obvious requires each model to have at least one uniquely identifying "ID" that can serve as its primary key. "Composite" primary keys are not supported by Obvious models. However, you are free to add additional multi-column, unique indexes to your database tables in addition to the table's uniquely identifying primary key.

<a name="uuid-and-ulid-keys"></a>
### UUID and ULID Keys

Instead of using auto-incrementing integers as your Obvious model's primary keys, you may choose to use UUIDs instead. UUIDs are universally unique alpha-numeric identifiers that are 36 characters long.

If you would like a model to use a UUID key instead of an auto-incrementing integer key, you may use the `MacropaySolutions\Kernel\Database\Obvious\Concerns\HasUuids` trait on the model. Of course, you should ensure that the model has a [UUID equivalent primary key column](/migrations#column-method-uuid):

    use MacropaySolutions\Kernel\Database\Obvious\Concerns\HasUuids;
    use MacropaySolutions\Kernel\Database\Obvious\Model;
...
By default, The `HasUuids` trait will generate ["ordered" UUIDs](/strings#method-str-ordered-uuid) for your models. These UUIDs are more efficient for indexed database storage because they can be sorted lexicographically.
...
If you wish, you may choose to utilize "ULIDs" instead of UUIDs. ULIDs are similar to UUIDs; however, they are only 26 characters in length. Like ordered UUIDs, ULIDs are lexicographically sortable for efficient database indexing. To utilize ULIDs, you should use the `MacropaySolutions\Kernel\Database\Obvious\Concerns\HasUlids` trait on your model. You should also ensure that the model has a [ULID equivalent primary key column](/migrations#column-method-ulid):

    use MacropaySolutions\Kernel\Database\Obvious\Concerns\HasUlids;
    use MacropaySolutions\Kernel\Database\Obvious\Model;
...
By default, all Obvious models will use the default database connection that is configured for your application. If you would like to specify a different connection that should be used when interacting with a particular model, you should define a `$connection` property on the model:
...
<a name="configuring-obvious-strictness"></a>
### Configuring Obvious Strictness

Kernel offers several methods that allow you to configure Obvious's behavior and "strictness" in a variety of situations.
...
Once you have created a model and [its associated database table](/migrations#writing-migrations), you are ready to start retrieving data from your database. You can think of each Obvious model as a powerful [query builder](/queries) allowing you to fluently query the database table associated with the model. The model's `all` method will retrieve all the records from the model's associated database table:
...
> [!NOTE]  
> Since Obvious models are query builders, you should review all the methods provided by Kernel's [query builder](/queries). You may use any of these methods when writing your Obvious queries.
...
The `refresh` method will re-hydrate the existing model using fresh data from the database. In addition, all of its loaded relationships will be refreshed as well:
    ...
    $flight->a->number; // "FR 900"

<a name="collections"></a>
### Collections

As we have seen, Obvious methods like `all` and `get` retrieve multiple records from the database. However, these methods don't return a plain PHP array. Instead, an instance of `MacropaySolutions\Kernel\Database\Obvious\Collection` is returned.
...
In addition to the methods provided by Kernel's base collection class, the Obvious collection class provides [a few extra methods](/obvious-collections#available-methods) that are specifically intended for interacting with collections of Obvious models.

Since all of Kernel's collections implement PHP's iterable interfaces, you may loop over collections as if they were an array:
...
Your application may run out of memory if you attempt to load tens of thousands of Obvious records via the `all` or `get` methods. Instead of using these methods, the `chunk` method may be used to process large numbers of models more efficiently.

The `chunk` method will retrieve a subset of Obvious models, passing them to a closure for processing. Since only the current chunk of Obvious models is retrieved at a time, the `chunk` method will provide significantly reduced memory usage when working with a large number of models:
...
The `lazy` method works similarly to [the `chunk` method](#chunking-results) in the sense that, behind the scenes, it executes the query in chunks. However, instead of passing each chunk directly into a callback as is, the `lazy` method returns a flattened [`LazyCollection`](/collections#lazy-collections) of Obvious models, which lets you interact with the results as a single stream:
...
Similar to the `lazy` method, the `cursor` method may be used to significantly reduce your application's memory consumption when iterating through tens of thousands of Obvious model records.

The `cursor` method will only execute a single database query; however, the individual Obvious models will not be hydrated until they are actually iterated over. Therefore, only one Obvious model is kept in memory at any given time while iterating over the cursor.

> [!WARNING]  
> Since the `cursor` method only ever holds a single Obvious model in memory at a time, it cannot eager load relationships. If you need to eager load relationships, consider using [the `lazy` method](#chunking-using-lazy-collections) instead.
...
The `cursor` returns an `MacropaySolutions\Kernel\Support\LazyCollection` instance. [Lazy collections](/collections#lazy-collections) allow you to use many of the collection methods available on typical Kernel collections while only loading a single model into memory at a time:
...
Although the `cursor` method uses far less memory than a regular query (by only holding a single Obvious model in memory at a time), it will still eventually run out of memory. This is [due to PHP's PDO driver internally caching all raw query results in its buffer](https://www.php.net/manual/en/mysqlinfo.concepts.buffering.php). If you're dealing with a very large number of Obvious records, consider using [the `lazy` method](#chunking-using-lazy-collections) instead.
...
Obvious also offers advanced subquery support, which allows you to pull information from related tables in a single query. For example, let's imagine that we have a table of flight `destinations` and a table of `flights` to destinations. The `flights` table contains an `arrived_at` column which indicates when the flight arrived at the destination.

Using the subquery functionality available to the query builder's `select` and `addSelect` methods, we can select all the `destinations` and the name of the flight that most recently arrived at that destination using a single query:
...
In addition to retrieving all the records matching a given query, you may also retrieve single records using the `find`, `first`, or `firstWhere` methods. Instead of returning a collection of models, these methods return a single model instance:
...
Of course, when using Obvious, we don't only need to retrieve models from the database. We also need to insert new records. Thankfully, Obvious makes it simple. To insert a new record into the database, you should instantiate a new model instance and set attributes on the model. Then, call the `save` method on the model instance:
...
However, before using the `create` method, you will need to specify either a `fillable` or `guarded` property on your model class. These properties are required because all Obvious models are protected against mass assignment vulnerabilities by default. To learn more about mass assignment, please consult the [mass assignment documentation](#mass-assignment).
...
> [!WARNING]  
> When issuing a mass update via Obvious, the `saving`, `saved`, `updating`, and `updated` model events will not be fired for the updated models. This is because the models are never actually retrieved when issuing a mass update.

<a name="examining-attribute-changes"></a>
#### Examining Attribute Changes

Obvious provides the `isDirty`, `isClean`, and `wasChanged` methods to examine the internal state of your model and determine how its attributes have changed from when the model was originally retrieved.
...
However, before using the `create` method, you will need to specify either a `fillable` or `guarded` property on your model class. These properties are required because all Obvious models are protected against mass assignment vulnerabilities by default.
...
If you would like to make all of your attributes mass assignable, you may define your model's `$guarded` property as an empty array. If you choose to unguard your model, you should take special care to always hand-craft the arrays passed to Obvious's `fill`, `create`, and `update` methods:
...
If you wish, you may instruct Kernel to throw an exception when attempting to fill an unfillable attribute by invoking the `preventSilentlyDiscardingAttributes` method. Typically, this method should be invoked within the `boot` method of one of your application's service providers:
...
In addition to actually removing records from your database, Obvious can also "soft delete" models. When models are soft deleted, they are not actually removed from your database. Instead, a `deleted_at` attribute is set on the model indicating the date and time at which the model was "deleted". To enable soft deletes for a model, add the `MacropaySolutions\Kernel\Database\Obvious\SoftDeletes` trait to the model:
...
You should also add the `deleted_at` column to your database table. The Kernel [schema builder](/migrations) contains a helper method to create this column:
    ...
    \app('db')->connection()->getSchemaBuilder()->table('flights', function (Blueprint $table) {
        $table->softDeletes();
    });
...
The `restore` method may also be used when building [relationship](/obvious-relationships) queries:

    $flight->r->history()->restore();
...
You may also use the `forceDelete` method when building Obvious relationship queries:

    $flight->r->history()->forceDelete();
...
The `withTrashed` method may also be called when building a [relationship](/obvious-relationships) query:

    $flight->r->history()->withTrashed()->get();
...
Sometimes you may want to periodically delete models that are no longer needed. To accomplish this, you may add the `MacropaySolutions\Kernel\Database\Obvious\Prunable` or `MacropaySolutions\Kernel\Database\Obvious\MassPrunable` trait to the models you would like to periodically prune. After adding one of the traits to the model, implement a `prunable` method which returns an Obvious query builder that resolves the models that are no longer needed:
...
After configuring your prunable model, you should schedule the `model:prune` run command in your application's `App\Console\Kernel` class. You are free to choose the appropriate interval at which this command should be run:
...
When models are marked with the `MacropaySolutions\Kernel\Database\Obvious\MassPrunable` trait, models are deleted from the database using mass-deletion queries. Therefore, the `pruning` method will not be invoked, nor will the `deleting` and `deleted` model events be dispatched. This is because the models are never actually retrieved before deletion, thus making the pruning process much more efficient:
...
Global scopes allow you to add constraints to all queries for a given model. Kernel's own [soft delete](#soft-deleting) functionality utilizes global scopes to only retrieve "non-deleted" models from the database. Writing your own global scopes can provide a convenient, easy way to make sure every query for a given model receives certain constraints.

<a name="generating-scopes"></a>
#### Generating Scopes

To generate a new global scope, you may invoke the `make:scope` run command, which will place the generated scope in your application's `app/Models/Scopes` directory:

```shell
php run make:scope AncientScope
```

<a name="writing-global-scopes"></a>
#### Writing Global Scopes

Writing a global scope is simple. First, use the `make:scope` command to generate a class that implements the `MacropaySolutions\Kernel\Database\Obvious\Scope` interface. The `Scope` interface requires you to implement one method: `apply`. The `apply` method may add `where` constraints or other types of clauses to the query as needed:
...
Obvious also allows you to define global scopes using closures, which is particularly useful for simple scopes that do not warrant a separate class of their own. When defining a global scope using a closure, you should provide a scope name of your own choosing as the first argument to the `addGlobalScope` method:
...
> [!NOTE]  
> Want to broadcast your Obvious events directly to your client-side application? Check out Kernel's [model event broadcasting](/broadcasting#model-broadcasting).

Obvious models dispatch several events, allowing you to hook into the following moments in a model's lifecycle: `retrieved`, `creating`, `created`, `updating`, `updated`, `saving`, `saved`, `deleting`, `deleted`, `trashed`, `forceDeleting`, `forceDeleted`, `restoring`, `restored`, and `replicating`.

The `retrieved` event will dispatch when an existing model is retrieved from the database. When a new model is saved for the first time, the `creating` and `created` events will dispatch. The `updating` / `updated` events will dispatch when an existing model is modified and the `save` method is called. The `saving` / `saved` events will dispatch when a model is created or updated - even if the model's attributes have not been changed. Event names ending with `-ing` are dispatched before any changes to the model are persisted, while events ending with `-ed` are dispatched after the changes to the model are persisted.

To start listening to model events, define a `$dispatchesEvents` property on your Obvious model. This property maps various points of the Obvious model's lifecycle to your own [event classes](/events). Each model event class should expect to receive an instance of the affected model via its constructor:
...
After defining and mapping your Obvious events, you may use [event listeners](/events#defining-listeners) to handle the events.

> [!WARNING]  
> When issuing a mass update or delete query via Obvious, the `saved`, `updated`, `deleting`, and `deleted` model events will not be dispatched for the affected models. This is because the models are never actually retrieved when performing mass updates or deletes.
...
If you are listening for many events on a given model, you may use observers to group all of your listeners into a single class. Observer classes have method names which reflect the Obvious events you wish to listen for. Each of these methods receives the affected model as their only argument. The `make:observer` run command is the easiest way to create a new observer class:

```shell
php run make:observer UserObserver --model=User
```

This command will place the new observer in your `app/Observers` directory. If this directory does not exist, run will create it for you. Your fresh observer will look like the following:
...
By default, `php-framework` will not automatically discover and register your observers from the cache. You can control this behavior by overriding the `shouldDiscoverEventsAsObservers` method in your application's `App\Providers\EventServiceProvider` service provider.

If you choose to not enable auto-discovery, you must manually register your observers by calling the `observe` method on the model you wish to observe within the `boot` method of your service provider:
...
> {!NOTE]
> Kernel fires `-ing` post-fixed events **before commit** when `ShouldHandleEventsAfterCommit` or `$afterCommit = true` are used in observers!
