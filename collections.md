---
title: Collections
description: Powerful wrappers for working with arrays of data in PHP Framework.
context: collections
---
# Collections
...
The `MacropaySolutions\Kernel\Support\Collection` class provides a fluent, convenient wrapper for working with arrays of data. For example, check out the following code. We'll use the `collect` helper to create a new collection instance from the array, run the `strtoupper` function on each element, and then remove all empty elements:

    $collection = collect(['surname', 'abigail', null])->map(function (?string $name) {
...
As mentioned above, the `collect` helper returns a new `MacropaySolutions\Kernel\Support\Collection` instance for the given array. So, creating a collection is as simple as:

    $collection = collect([1, 2, 3]);

> [!NOTE]  
> The results of [Obvious](/obvious) queries are always returned as `Collection` instances.
...
The `collect` method is primarily useful for converting [lazy collections](#lazy-collections) into standard `Collection` instances:
    ...
    // 'MacropaySolutions\Kernel\Support\Collection'
...
> [!NOTE]  
> This method's behavior is modified when using [Obvious Collections](/obvious-collections#method-contains).
...
> [!NOTE]  
> This method's behavior is modified when using [Obvious Collections](/obvious-collections#method-diff).
...
> [!NOTE]  
> This method's behavior is modified when using [Obvious Collections](/obvious-collections#method-except).
...
The `firstOrFail` method is identical to the `first` method; however, if no result is found, an `MacropaySolutions\Kernel\Support\ItemNotFoundException` exception will be thrown:
...
You may also call the `firstOrFail` method with no arguments to get the first element in the collection. If the collection is empty, an `MacropaySolutions\Kernel\Support\ItemNotFoundException` exception will be thrown:
...
The `flip` method swaps the collection's keys with their corresponding values:
    ...
    $collection = collect(['name' => 'surname', 'framework' => 'framework']);

    $flipped = $collection->flip();

    $flipped->all();

    // ['surname' => 'name', 'framework' => 'framework']
...
> [!NOTE]  
> This method's behavior is modified when using [Obvious Collections](/obvious-collections#method-intersect).

<a name="method-intersectAssoc"></a>
#### `intersectAssoc()` {.collection-method}

The `intersectAssoc` method compares the original collection against another collection or `array`, returning the key / value pairs that are present in all the given collections:
...
The `lazy` method returns a new [`LazyCollection`](#lazy-collections) instance from the underlying array of items:
    ...
    // MacropaySolutions\Kernel\Support\LazyCollection
...
> [!NOTE]  
> This method's behavior is modified when using [Obvious Collections](/obvious-collections#method-only).
...
The `pluck` method retrieves all the values for a given key:
...
The `replace` method behaves similarly to `merge`; however, in addition to overwriting matching items that have string keys, the `replace` method will also overwrite items in the collection that have matching numeric keys:
    ...
    $collection = collect(['Surname', 'Abigail', 'James']);

    $replaced = $collection->replace([1 => 'Victoria', 3 => 'Finn']);

    $replaced->all();

    // ['Surname', 'Victoria', 'James', 'Finn']
...
```php
$users = collect([
    ['name' => 'Surname Name', 'role' => 'Developer', 'status' => 'active'],
...
/*
    [
        ['name' => 'Surname Name', 'role' => 'Developer'],
...
If there are no elements in the collection that should be returned by the `sole` method, an `\MacropaySolutions\Kernel\Collections\ItemNotFoundException` exception will be thrown. If there is more than one element that should be returned, an `\MacropaySolutions\Kernel\Collections\MultipleItemsFoundException` will be thrown.
...
If you would like to sort your collection by multiple attributes, you may pass an array of the attributes that you wish to sort by:
    ...
    $collection = collect([
        ['name' => 'Surname Name', 'age' => 34],
        ['name' => 'Abigail Name', 'age' => 30],
        ['name' => 'Surname Name', 'age' => 36],
        ['name' => 'Abigail Name', 'age' => 32],
    ]);
...
The `tap` method passes the collection to the given callback, allowing you to "tap" into the collection at a specific point and do something with the items while not affecting the collection itself. The collection is then returned by the `tap` method:
    ...
    collect([2, 4, 3, 1, 5])
        ...
        ->tap(function (Collection $collection) {
            \app('log')->debug('Values after sorting', $collection->values()->all());
        })
        ->shift();
...
The `toArray` method converts the collection into a plain PHP `array`. If the collection's values are [Obvious](/obvious) models, the models will also be converted to arrays:
...
> [!WARNING]  
> `toArray` also converts all the collection's nested objects that are an instance of `Arrayable` to an array. If you want to get the raw array underlying the collection, use the [`all`](#method-all) method instead.
...
The `unique` method returns all the unique items in the collection. The returned collection keeps the original array keys, so in the following example we will use the [`values`](#method-values) method to reset the keys to consecutively numbered indexes:
...
> [!NOTE]  
> This method's behavior is modified when using [Obvious Collections](/obvious-collections#method-unique).
...
A second closure may be passed to the `whenEmpty` method that will be executed when the collection is not empty:
    ...
    $collection->whenEmpty(function (Collection $collection) {
        ...
        return $collection->push('Surname');
    });
...
> [!WARNING]  
> Before learning more about Framework's lazy collections, take some time to familiarize yourself with [PHP generators](https://www.php.net/manual/en/language.generators.overview.php).
...
For example, imagine your application needs to process a multi-gigabyte log file while taking advantage of Framework's collection methods to parse the logs. Instead of reading the entire file into memory at once, lazy collections may be used to keep only a small part of the file in memory at a given time:
...
Or, imagine you need to iterate through 10,000 Obvious models. When using traditional Framework collections, all 10,000 Obvious models must be loaded into memory at the same time:
...
However, the query builder's `cursor` method returns a `LazyCollection` instance. This allows you to still only run a single query against the database but also only keep one Obvious model loaded in memory at a time. In this example, the `filter` callback is not executed until we actually iterate over each user individually, allowing for a drastic reduction in memory usage:
...
Almost all methods available on the `Collection` class are also available on the `LazyCollection` class. Both of these classes implement the `MacropaySolutions\Kernel\Support\Enumerable` contract, which defines the following methods:
...
To illustrate the usage of this method, imagine an application that submits invoices from the database using a cursor. You could define a [scheduled task](/scheduling) that runs every 15 minutes and only processes invoices for a maximum of 14 minutes:
