...
description: Paginating query builder and Obvious ORM database results efficiently in PHP Framework.
context: pagination
---
# Database: Pagination
...
- [Basic Usage](#basic-usage)
    ...
    - [Paginating Obvious Results](#paginating-obvious-results)
...
In other frameworks, pagination can be very painful. We hope Framework's approach to pagination will be a breath of fresh air. Framework's paginator is integrated with the [query builder](/queries) and [Obvious ORM](/obvious) and provides convenient, easy-to-use pagination of database records with zero configuration.
...
If you are using Framework's default Tailwind pagination views and the Tailwind JIT engine, you should ensure your application's `tailwind.config.js` file's `content` key references Framework's pagination views so that their Tailwind classes are not purged:
...
content: [
    ...
    './resources/**/*.js',
    './resources/**/*.vue',
    './vendor/macropay-solutions/php-kernel/kernel/MacropaySolutions/Kernel/Pagination/resources/views/*.template.php',
],
```
...
There are several ways to paginate items. The simplest is by using the `paginate` method on the [query builder](/queries) or an [Obvious query](/obvious). The `paginate` method automatically takes care of setting the query's "limit" and "offset" based on the current page being viewed by the user. By default, the current page is detected by the value of the `page` query string argument on the HTTP request. This value is automatically detected by Framework, and is also automatically inserted into links generated by the paginator.
    ...
    class UserController extends Controller
    {
        ...
        public function index(): View
        {
            return view('user.index', [
                'users' => app('db')->table('users')->paginate(15)
            ]);
        }
    }
...
Therefore, if you only need to display simple "Next" and "Previous" links in your application's UI, you may use the `simplePaginate` method to perform a single, efficient query:
    ...
    $users = app('db')->table('users')->simplePaginate(15);

<a name="paginating-obvious-results"></a>
### Paginating Obvious Results

You may also paginate [Obvious](/obvious) queries. In this example, we will paginate the `App\Models\User` model and indicate that we plan to display 15 records per page. As you can see, the syntax is nearly identical to paginating query builder results:
...
You may also use the `simplePaginate` method when paginating Obvious models:

    $users = User::where('votes', '>', 100)->simplePaginate(15);

Similarly, you may use the `cursorPaginate` method to cursor paginate Obvious models:
...
You may create a cursor based paginator instance via the `cursorPaginate` method offered by the query builder. This method returns an instance of `MacropaySolutions\Kernel\Pagination\CursorPaginator`:

    $users = app('db')->table('users')->orderBy('id')->cursorPaginate(15);
...
Sometimes you may wish to create a pagination instance manually, passing it an array of items that you already have in memory. You may do so by creating either an `MacropaySolutions\Kernel\Pagination\Paginator`, `MacropaySolutions\Kernel\Pagination\LengthAwarePaginator` or `MacropaySolutions\Kernel\Pagination\CursorPaginator` instance, depending on your needs.
...
You may use the `withQueryString` method if you would like to append all the current request's query string values to the pagination links:
...
When calling the `paginate` method, you will receive an instance of `MacropaySolutions\Kernel\Pagination\LengthAwarePaginator`, while calling the `simplePaginate` method returns an instance of `MacropaySolutions\Kernel\Pagination\Paginator`. And, finally, calling the `cursorPaginate` method returns an instance of `MacropaySolutions\Kernel\Pagination\CursorPaginator`.

These objects provide several methods that describe the result set. In addition to these helper methods, the paginator instances are iterators and may be looped as an array. So, once you have retrieved the results, you may display the results and render the page links using [Template](/template):
...
The JSON from the paginator will include meta information such as `total`, `current_page`, `last_page`, and more. The result records are available via the `data` key in the JSON array. Here is an example of the JSON created by returning a paginator instance from a route:

    {
       ...
       "current_page": 1,
       "last_page": 4,
       "first_page_url": "http://framework.app?page=1",
       "last_page_url": "http://framework.app?page=4",
       "next_page_url": "http://framework.app?page=2",
       "prev_page_url": null,
       "path": "http://framework.app",
       "from": 1,
       "to": 15,
       "data":[
            {
...
```shell
php run vendor:publish --tag=framework-pagination
```

This command will place the views in your application's `resources/views/vendor/pagination` directory. The `tailwind.template.php` file within this directory corresponds to the default pagination view. You may edit this file to modify the pagination HTML.
...
Framework includes pagination views built using [Bootstrap CSS](https://getbootstrap.com/). To use these views instead of the default Tailwind views, you may call the paginator's `useBootstrapFour` or `useBootstrapFive` methods within the `boot` method of your `App\Providers\AppServiceProvider` class:
