---
title: File Storage
description: Managing file storage and disks in PHP Framework.
context: filesystem
---
# File Storage
...
Framework provides a powerful filesystem abstraction thanks to the wonderful [Flysystem](https://github.com/thephpleague/flysystem) PHP package by Frank de Jonge. The Framework Flysystem integration provides simple drivers for working with local filesystems, SFTP, and Amazon S3. Even better, it's amazingly simple to switch between these storage options between your local development machine and production server as the API remains the same for each system.

<a name="configuration"></a>
## Configuration

Framework's filesystem configuration file is located at `config/filesystems.php`. Within this file, you may configure all of your filesystem "disks". Each disk represents a particular storage driver and storage location. Example configurations for each supported driver are included in the configuration file so you can modify the configuration to reflect your storage preferences and credentials.

The `local` driver interacts with files stored locally on the server running the Framework application while the `s3` driver is used to write to Amazon's S3 cloud storage service.
...
When using the `local` driver, all file operations are relative to the `root` directory defined in your `filesystems` configuration file. By default, this value is set to the `storage/app` directory. Therefore, the following method would write to `storage/app/example.txt`:
    ...
    app('filesystem')->disk('local')->put('example.txt', 'Contents');
...
To make these files accessible from the web, you should create a symbolic link from `public/storage` to `storage/app/public`. Utilizing this folder convention will keep your publicly accessible files in one directory that can be easily shared across deployments when using zero down-time deployment systems.

To create the symbolic link, you may use the `storage:link` Run command:

```shell
php run storage:link
```
...
Framework's Flysystem integrations work great with FTP; however, a sample configuration is not included with the framework's default `filesystems.php` configuration file. If you need to configure an FTP filesystem, you may use the configuration example below:
...
Framework's Flysystem integrations work great with SFTP; however, a sample configuration is not included with the framework's default `filesystems.php` configuration file. If you need to configure an SFTP filesystem, you may use the configuration example below:
...
In order for Framework's Flysystem integration to generate proper URLs when using MinIO, you should define the `AWS_URL` environment variable so that it matches your application's local URL and includes the bucket name in the URL path:
...
The `app('filesystem')` may be used to interact with any of your configured disks. For example, you may use the `put` method on it to store an avatar on the default disk. If you call methods on the `app('filesystem')` without first calling the `disk` method, the method will automatically be passed to the default disk:



    app('filesystem')->put('avatars/1', $content);

If your application interacts with multiple disks, you may use the `disk` method on the `app('filesystem')` to work with files on a particular disk:

    app('filesystem')->disk('s3')->put('avatars/1', $content);

<a name="on-demand-disks"></a>
### On-Demand Disks

Sometimes you may wish to create a disk at runtime using a given configuration without that configuration actually being present in your application's `filesystems` configuration file. To accomplish this, you may pass a configuration array to the `app('filesystem')`'s `build` method:

```php

$disk = app('filesystem')->build([
    'driver' => 'local',
    'root' => '/path/to/root',
]);
...
The `get` method may be used to retrieve the contents of a file. The raw string contents of the file will be returned by the method. Remember, all file paths should be specified relative to the disk's "root" location:
    ...
    $contents = app('filesystem')->get('file.jpg');

If the file you are retrieving contains JSON, you may use the `json` method to retrieve the file and decode its contents:

    $orders = app('filesystem')->json('orders.json');

The `exists` method may be used to determine if a file exists on the disk:

    if (app('filesystem')->disk('s3')->exists('file.jpg')) {
        // ...
    }

The `missing` method may be used to determine if a file is missing from the disk:

    if (app('filesystem')->disk('s3')->missing('file.jpg')) {
        // ...
    }
...
The `download` method may be used to generate a response that forces the user's browser to download the file at the given path. The `download` method accepts a filename as the second argument to the method, which will determine the filename that is seen by the user downloading the file. Finally, you may pass an array of HTTP headers as the third argument to the method:
    ...
    return app('filesystem')->download('file.jpg');

    return app('filesystem')->download('file.jpg', $name, $headers);
...
You may use the `url` method to get the URL for a given file. If you are using the `local` driver, this will typically just prepend `/storage` to the given path and return a relative URL to the file. If you are using the `s3` driver, the fully qualified remote URL will be returned:
    ...
    $url = app('filesystem')->url('file.jpg');
...
If you would like to pre-define the host for URLs generated using the `app('filesystem')`, you may add a `url` option to the disk's configuration array:
...
Using the `temporaryUrl` method, you may create temporary URLs to files stored using the `s3` driver. This method accepts a path and a `DateTime` instance specifying when the URL should expire:
    ...
    $url = app('filesystem')->temporaryUrl(
...
If you need to customize how temporary URLs are created for a specific storage disk, you can use the `buildTemporaryUrlsUsing` method. For example, this can be useful if you have a controller that allows you to download files stored via a disk that doesn't typically support temporary URLs. Usually, this method should be called from the `boot` method of a service provider:
    ...
    class AppServiceProvider extends ServiceProvider
    {
        ...
        {
            app('filesystem')->disk('local')->buildTemporaryUrlsUsing(
...
If you need to generate a temporary URL that can be used to upload a file directly from your client-side application, you may use the `temporaryUploadUrl` method. This method accepts a path and a `DateTime` instance specifying when the URL should expire. The `temporaryUploadUrl` method returns an associative array which may be destructured into the upload URL and the headers that should be included with the upload request:
    ...
    ['url' => $url, 'headers' => $headers] = app('filesystem')->temporaryUploadUrl(
        'file.jpg', now()->addMinutes(5)
    );
...
In addition to reading and writing files, Framework can also provide information about the files themselves. For example, the `size` method may be used to get the size of a file in bytes:



    $size = app('filesystem')->size('file.jpg');

The `lastModified` method returns the UNIX timestamp of the last time the file was modified:

    $time = app('filesystem')->lastModified('file.jpg');

The MIME type of a given file may be obtained via the `mimeType` method:

    $mime = app('filesystem')->mimeType('file.jpg');
...
You may use the `path` method to get the path for a given file. If you are using the `local` driver, this will return the absolute path to the file. If you are using the `s3` driver, this method will return the relative path to the file in the S3 bucket:
    ...
    $path = app('filesystem')->path('file.jpg');
...
The `put` method may be used to store file contents on a disk. You may also pass a PHP `resource` to the `put` method, which will use Flysystem's underlying stream support. Remember, all file paths should be specified relative to the "root" location configured for the disk:
    ...
    app('filesystem')->put('file.jpg', $contents);

    app('filesystem')->put('file.jpg', $resource);
...
If the `put` method (or other "write" operations) is unable to write the file to disk, `false` will be returned:
    ...
    if (!app('filesystem')->put('file.jpg', $contents)) {
...
The `prepend` and `append` methods allow you to write to the beginning or end of a file:
    ...
    app('filesystem')->prepend('file.log', 'Prepended Text');

    app('filesystem')->append('file.log', 'Appended Text');
...
The `copy` method may be used to copy an existing file to a new location on the disk, while the `move` method may be used to rename or move an existing file to a new location:
    ...
    app('filesystem')->copy('old/file.jpg', 'new/file.jpg');

    app('filesystem')->move('old/file.jpg', 'new/file.jpg');

<a name="automatic-streaming"></a>
### Automatic Streaming

Streaming files to storage offers significantly reduced memory usage. If you would like Framework to automatically manage streaming a given file to your storage location, you may use the `putFile` or `putFileAs` method. This method accepts either an `MacropaySolutions\Kernel\Http\File` or `MacropaySolutions\Kernel\Http\UploadedFile` instance and will automatically stream the file to your desired location:

    use MacropaySolutions\Kernel\Http\File;


    // Automatically generate a unique ID for filename...
    $path = app('filesystem')->putFile('photos', new File('/path/to/photo'));

    // Manually specify a filename...
    $path = app('filesystem')->putFileAs('photos', new File('/path/to/photo'), 'photo.jpg');
...
The `putFile` and `putFileAs` methods also accept an argument to specify the "visibility" of the stored file. This is particularly useful if you are storing the file on a cloud disk such as Amazon S3 and would like the file to be publicly accessible via generated URLs:
    ...
    app('filesystem')->putFile('photos', new File('/path/to/photo'), 'public');

<a name="file-uploads"></a>
### File Uploads

In web applications, one of the most common use-cases for storing files is storing user uploaded files such as photos and documents. Framework makes it very easy to store uploaded files using the `store` method on an uploaded file instance. Call the `store` method with the path at which you wish to store the uploaded file:
...
You may also call the `putFile` method on the `app('filesystem')` to perform the same file storage operation as the example above:

    $path = app('filesystem')->putFile('avatars', $request->file('avatar'));
...
You may also use the `putFileAs` method on the `app('filesystem')`, which will perform the same file storage operation as the example above:

    $path = app('filesystem')->putFileAs(
        'avatars', $request->file('avatar'), $request->user()->id
    );

> [!WARNING]  
> Unprintable and invalid unicode characters will automatically be removed from file paths. Therefore, you may wish to sanitize your file paths before passing them to Framework's file storage methods. File paths are normalized using the `League\Flysystem\WhitespacePathNormalizer::normalizePath` method.
...
In Framework's Flysystem integration, "visibility" is an abstraction of file permissions across multiple platforms. Files may either be declared `public` or `private`. When a file is declared `public`, you are indicating that the file should generally be accessible to others. For example, when using the S3 driver, you may retrieve URLs for `public` files.

You can set the visibility when writing the file via the `put` method:



    app('filesystem')->put('file.jpg', $contents, 'public');

If the file has already been stored, its visibility can be retrieved and set via the `getVisibility` and `setVisibility` methods:

    $visibility = app('filesystem')->getVisibility('file.jpg');

    app('filesystem')->setVisibility('file.jpg', 'public');
...
The `delete` method accepts a single filename or an array of files to delete:
    ...
    app('filesystem')->delete('file.jpg');

    app('filesystem')->delete(['file.jpg', 'file2.jpg']);

If necessary, you may specify the disk that the file should be deleted from:



    app('filesystem')->disk('s3')->delete('path/file.jpg');
...
The `files` method returns an array of all the files in a given directory. If you would like to retrieve a list of all files within a given directory including all subdirectories, you may use the `allFiles` method:



    $files = app('filesystem')->files($directory);

    $files = app('filesystem')->allFiles($directory);
...
The `directories` method returns an array of all the directories within a given directory. Additionally, you may use the `allDirectories` method to get a list of all directories within a given directory and all of its subdirectories:
    ...
    $directories = app('filesystem')->directories($directory);

    $directories = app('filesystem')->allDirectories($directory);
...
Finally, the `deleteDirectory` method may be used to remove a directory and all of its files:
    ...
    app('filesystem')->deleteDirectory($directory);

<a name="testing"></a>
## Testing

The `app('filesystem')`'s `fake` method allows you to easily generate a fake disk that, combined with the file generation utilities of the `MacropaySolutions\Kernel\Http\UploadedFile` class, greatly simplifies the testing of file uploads. For example:
    ...
    class ExampleTest extends TestCase
    {
        ...
        {
            $fakeDisk = app('filesystem')->createLocalDriver([
                'root' => storage_path('framework/testing/disks/photos')
            ]);
            
            app('filesystem')->set('photos', $fakeDisk);
            ...
            app('filesystem')->disk('photos')->assertExists('photo1.jpg');
            app('filesystem')->disk('photos')->assertExists(['photo1.jpg', 'photo2.jpg']);

            // Assert one or more files were not stored...
            app('filesystem')->disk('photos')->assertMissing('missing.jpg');
            app('filesystem')->disk('photos')->assertMissing(['missing.jpg', 'non-existing.jpg']);

            // Assert that a given directory is empty...
            app('filesystem')->disk('photos')->assertDirectoryEmpty('/wallpapers');
        }
    }
...
Framework's Flysystem integration provides support for several "drivers" out of the box; however, Flysystem is not limited to these and has adapters for many other storage systems. You can create a custom driver if you want to use one of these additional adapters in your Framework application.
...
Next, you can register the driver within the `boot` method of one of your application's [service providers](/providers). To accomplish this, you should use the `extend` method of the `app('filesystem')`:
    ...
    class AppServiceProvider extends ServiceProvider
    {
        ...
        public function boot(): void
        {
            app('filesystem')->extend('dropbox', function (Application $app, array $config) {
...
The first argument of the `extend` method is the name of the driver and the second is a closure that receives the `$app` and `$config` variables. The closure must return an instance of `MacropaySolutions\Kernel\Filesystem\FilesystemAdapter`. The `$config` variable contains the values defined in `config/filesystems.php` for the specified disk.
