...
If no path is provided to the `url` helper, an `MacropaySolutions\Kernel\Routing\UrlGenerator` instance is returned, allowing you to access information about the current URL:
...
Framework allows you to easily create "signed" URLs to named routes. These URLs have a "signature" hash appended to the query string which allows Framework to verify that the URL has not been modified since it was created. Signed URLs are especially useful for routes that are publicly accessible yet need a layer of protection against URL manipulation.

For example, you might use signed URLs to implement a public "unsubscribe" link that is emailed to your customers. To create a signed URL to a named route, use the `signedRoute` method of the `url` service:

    return app('url')->signedRoute('unsubscribe', ['user' => 1]);

You may exclude the domain from the signed URL hash by providing the `absolute` argument to the `signedRoute` method:

    return app('url')->signedRoute('unsubscribe', ['user' => 1], absolute: false);

If you would like to generate a temporary signed route URL that expires after a specified amount of time, you may use the `temporarySignedRoute` method. When Framework validates a temporary signed route URL, it will ensure that the expiration timestamp that is encoded into the signed URL has not elapsed:
...
To verify that an incoming request has a valid signature, you should call the `hasValidSignature` method on the incoming `MacropaySolutions\Kernel\Http\Request` instance:

    use MacropaySolutions\Kernel\Http\Request;

    $router->get('/unsubscribe/{user}', ['as' => 'unsubscribe', 'uses' => 'SubscriptionController@unsubscribe']);
...
Instead of validating signed URLs using the incoming request instance, you may assign the `MacropaySolutions\Kernel\Routing\Middleware\ValidateSignature` [middleware](/middleware) to the route. If it is not already present, you may assign this middleware an alias in your HTTP kernel's `$middlewareAliases` array:
    ...
    protected $middlewareAliases = [
        'signed' => \MacropaySolutions\Kernel\Routing\Middleware\ValidateSignature::class,
    ];
...
Once you have registered the middleware in your kernel, you may attach it to a route. If the incoming request does not have a valid signature, the middleware will automatically return a `403` HTTP response:
    ...
    $router->post('/unsubscribe/{user}', [
        'as' => 'unsubscribe', 'uses' => 'SubscriptionController@unsubscribe', 'middleware' => 'signed'
    ]);

If your signed URLs do not include the domain in the URL hash, you should provide the `relative` argument to the middleware:

    $router->post('/unsubscribe/{user}', [
        'as' => 'unsubscribe', 'uses' => 'SubscriptionController@unsubscribe', 'middleware' => 'signed:relative'
    ]);
...
When someone visits a signed URL that has expired, they will receive a generic error page for the `403` HTTP status code. However, you can customize this behavior by defining a custom "renderable" closure for the `InvalidSignatureException` exception in your exception handler. This closure should return an HTTP response:
    ...
    use MacropaySolutions\Kernel\Routing\Exceptions\InvalidSignatureException;
...
For some applications, you may wish to specify request-wide default values for certain URL parameters. For example, imagine many of your routes define a `{locale}` parameter:
    ...
    $router->get('/{locale}/posts', ['as' => 'post.index', 'uses' => 'PostController@index']);

It is cumbersome to always pass the `locale` every time you call the `route` helper. So, you may use the `app('url')->defaults` method to define a default value for this parameter that will always be applied during the current request. You may wish to call this method from a [route middleware](/middleware#assigning-middleware-to-routes) so that you have access to the current request:
    ...
    class SetDefaultLocaleForUrls
    {
        ...
        {
            app('url')->defaults(['locale' => $request->user()->locale]);
...
Setting URL default values can interfere with Framework's handling of implicit model bindings. Therefore, you should [prioritize your middleware](/middleware#sorting-middleware) that set URL defaults to be executed before Framework's own `SubstituteBindings` middleware. You can accomplish this by making sure your middleware occurs before the `SubstituteBindings` middleware within the `$middlewarePriority` property of your application's HTTP kernel.

The `$middlewarePriority` property is defined in the base `MacropaySolutions\Kernel\Http\Kernel` class. You may copy its definition from that class and overwrite it in your application's HTTP kernel in order to modify it:
    ...
    protected $middlewarePriority = [
        // ...
         ...
         \MacropaySolutions\Kernel\Routing\Middleware\SubstituteBindings::class,
         // ...
    ];
