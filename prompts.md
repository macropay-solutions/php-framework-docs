---
title: Prompts
description: Command-line forms and interactive console prompts for PHP Framework.
context: prompts
---
# Prompts
...
[Framework Prompts](/prompts) adds beautiful and user-friendly forms to your command-line applications, with browser-like features including placeholder text and validation.


Framework Prompts is perfect for accepting user input in your [Run console commands](/run#writing-commands), but it may also be used in any command-line PHP project.

> [!NOTE]  
> Framework Prompts supports macOS, Linux, and Windows with WSL. For more information, please see our documentation on [unsupported environments & fallbacks](#fallbacks).

<a name="installation"></a>
## Installation

Framework Prompts is already included with the latest release of Framework.
...
```php
use function MacropaySolutions\Prompts\text;
...
```php
use function MacropaySolutions\Prompts\password;
...
```php
use function MacropaySolutions\Prompts\multiselect;
...
```php
use function MacropaySolutions\Prompts\suggest;

$name = suggest('What is your name?', ['Surname', 'Dayle']);
```
...
$name = suggest(
    'What is your name?',
    fn ($value) => collect(['Surname', 'Dayle'])
...
```php
use function MacropaySolutions\Prompts\multisearch;
...
```php
use function MacropaySolutions\Prompts\pause;
...
```php
use function MacropaySolutions\Prompts\info;
...
```php
use function MacropaySolutions\Prompts\table;
...
$response = spin(
    fn () => app(\MacropaySolutions\Kernel\Http\Client\Factory::class)->get('[http://example.com](http://example.com)'),
    'Fetching response...'
);
```
...
For long running tasks, it can be helpful to show a progress bar that informs users how complete the task is. Using the `progress` function, Framework will display a progress bar and advance its progress for each iteration over a given iterable value:

```php
use function MacropaySolutions\Prompts\progress;
...
The callback may also accept the `\MacropaySolutions\Prompts\Progress` instance, allowing you to modify the label and hint on each iteration:
...
Framework Prompts supports macOS, Linux, and Windows with WSL. Due to limitations in the Windows version of PHP, it is not currently possible to use Framework Prompts on Windows outside of WSL.

For this reason, Framework Prompts supports falling back to an alternative implementation such as the [Symfony Console Question Helper](https://symfony.com/doc/current/components/console/helpers/questionhelper.html).

> [!NOTE]  
> When using Framework Prompts with the Framework framework, fallbacks for each prompt have been configured for you and will be automatically enabled in unsupported environments.

<a name="fallback-conditions"></a>
#### Fallback Conditions

If you are not using Framework or need to customize when the fallback behavior is used, you may pass a boolean to the `fallbackWhen` static method on the `Prompt` class:

```php
use MacropaySolutions\Prompts\Prompt;
...
If you are not using Framework or need to customize the fallback behavior, you may pass a closure to the `fallbackUsing` static method on each prompt class:
