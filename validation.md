---
title: Validation
description: Guide to validating incoming request data, creating form requests, and using built-in or custom validation rules in PHP-Framework applications.
context: basics
---
...
- [Form Request Validation](#form-request-validation)
  ...
  - [Form Request Autowiring](#form-request-autowiring)
  - [Manual Form Request Instantiation](#manual-form-request-instantiation)
...
Framework provides several different approaches to validate your application's incoming data. By default, Framework's base controller class uses a `ProvidesConvenienceMethods` trait which provides a convenient `$this->validate` method to validate incoming HTTP requests with a variety of powerful validation rules. It is most common to use the `validate` method available on all incoming HTTP requests. However, we will discuss other approaches to validation as well.
...
To learn about Framework's powerful validation features, let's look at a complete example of validating a form and displaying the error messages back to the user. By reading this high-level overview, you'll be able to gain a good general understanding of how to validate incoming request data using Framework:
...
<a name="the-this-validate-method"></a>
#### The `$this->validate` Method

The `$this->validate` helper which is available in Framework will always lead to a JSON response with the relevant error messages. Since Framework is tailored for stateless APIs, flashing errors to the session or returning redirect responses upon validation failure is not supported out of the box.
Of course, you are free to manually create validator instances using the `validator()->make` method.
...
> In PHP-Kernel, certain primitive type rules act as implicit "bail" rules. If any of the following rules fail for an attribute, validation for that attribute will stop immediately to prevent unnecessary processing or potential type errors in subsequent rules:
...
Because Framework is primarily designed for stateless API workflows the `$errors` view variable is not available in your views.

Should validation fail, the `$this->validate` helper will automatically throw a `MacropaySolutions\Kernel\Validation\ValidationException`. The framework's exception handler will catch this and automatically generate a `422 Unprocessable Entity` HTTP response containing a [JSON representation of the validation errors](#validation-error-response-format).

If you are building a frontend application, your JavaScript HTTP client (such as Axios or fetch) should catch this `422` response and display the corresponding JSON error messages to the user.

<a name="quick-customizing-the-error-messages"></a>
#### Customizing the Error Messages

Framework's built-in validation rules each have an error message that is located in your application's `lang/en/validation.php` file. If your application does not have a `lang` directory, you may instruct Framework to create it using the `lang:publish` Run command.
...
In addition, you may copy this file to another language directory to translate the messages for your application's language. To learn more about Framework localization, check out the complete [localization documentation](/localization).

> [!WARNING]  
> By default, the Framework application skeleton does not include the `lang` directory. If you would like to customize Framework's language files, you may publish them via the `lang:publish` Run command.

<a name="a-note-on-optional-fields"></a>
### A Note on Optional Fields

By default, Framework includes the `TrimStrings` and `ConvertEmptyStringsToNull` middleware in your application's global middleware stack. These middleware are listed in the stack by the `App\Http\Kernel` class. Because of this, you will often need to mark your "optional" request fields as `nullable` if you do not want the validator to consider `null` values as invalid. For example:
...
When your application throws a `MacropaySolutions\Kernel\Validation\ValidationException` exception and the incoming HTTP request is expecting a JSON response, Framework will automatically format the error messages for you and return a `422 Unprocessable Entity` HTTP response.
...
For more complex validation scenarios, you may wish to create a "form request". Form requests are custom request classes that encapsulate their own validation and authorization logic. To create a form request class, you may use the `make:request` Run CLI command:

```shell
php run make:request StorePostRequest
```

The generated form request class will be placed in the `app/Http/Requests` directory. If this directory does not exist, it will be created when you run the `make:request` command. Each form request generated by Framework has two methods: `authorize` and `rules`.
...
As you might have guessed, the `authorize` method is responsible for determining if the currently authenticated user can perform the action represented by the request, while the `rules` method returns the validation rules that should apply to the request's data:
     ...
     *
     * @return array<string, \MacropaySolutions\Kernel\Contracts\Validation\Rule|array|string>
     */
...
> [!NOTE]  
> You may type-hint any dependencies you require within the `rules` method's signature. They will automatically be resolved via the Framework [service container](/container).
...
<a name="form-request-autowiring"></a>
#### Form Request Autowiring

To use Form Requests with autowiring in the controller's method, the following code must be uncommented in your `\App\Application` class:
...
The reason this might be commented out by default is to avoid the execution overhead of those two condition checks on every container resolution if `FormRequest` is not actively used.

<a name="manual-form-request-instantiation"></a>
#### Manual Form Request Instantiation

The `\App\FormRequest` can also be used without the autowiring code above by leveraging [php-crufd-wizard](https://github.com/macropay-solutions/php-crufd-wizard) libraries within a base Controller class:
...
This pattern will allow also [php-crufd-wizard-decorator](https://github.com/macropay-solutions/php-crufd-wizard-decorator) library to dynamically decorate the error messages keys.
...
The `after` method should return an array of callables or closures which will be invoked after validation is complete. The given callables will receive an `MacropaySolutions\Kernel\Validation\Validator` instance, allowing you to raise additional error messages if necessary:
...
As noted, the array returned by the `after` method may also contain invokable classes. The `__invoke` method of these classes will receive an `MacropaySolutions\Kernel\Validation\Validator` instance:
...
> [!NOTE]  
> You may type-hint any dependencies you need within the `authorize` method's signature. They will automatically be resolved via the Framework [service container](/container).
...
Many of Framework's built-in validation rule error messages contain an `:attribute` placeholder. If you would like the `:attribute` placeholder of your validation message to be replaced with a custom attribute name, you may specify the custom names by overriding the `attributes` method. This method should return an array of attribute / name pairs:
...
Likewise, if you need to normalize any request data after validation is complete, you may use the `passedValidation` method:
    ...
    {
        $this->replace(['name' => 'Surname']);
    }

<a name="manually-creating-validators"></a>
## Manually Creating Validators

If you do not want to use the `validate` method on the request, you may create a validator instance manually using the `validator()` helper to retrieve the validation factory. The `make` method generates a new validator instance:
...
If needed, you may provide custom error messages that a validator instance should use instead of the default error messages provided by Framework. There are several ways to specify custom messages. First, you may pass the custom messages as the third argument to the `make` method:
...
Many of Framework's built-in error messages include an `:attribute` placeholder that is replaced with the name of the field or attribute under validation. To customize the values used to replace these placeholders for specific fields, you may pass an array of custom attributes as the fourth argument to the `make` method:
...
Sometimes you need to perform additional validation after your initial validation is complete. You can accomplish this using the validator's `after` method. The `after` method accepts a closure or an array of callables which will be invoked after validation is complete. The given callables will receive an `MacropaySolutions\Kernel\Validation\Validator` instance, allowing you to raise additional error messages if necessary:
...
As noted, the `after` method also accepts an array of callables, which is particularly convenient if your "after validation" logic is encapsulated in invokable classes, which will receive an `MacropaySolutions\Kernel\Validation\Validator` instance via their `__invoke` method:
...
Alternatively, you may call the `safe` method on a form request or validator instance. This method returns an instance of `MacropaySolutions\Kernel\Support\ValidatedInput`. This object exposes `only`, `except`, and `all` methods to retrieve a subset of the validated data or the entire array of validated data:
...
In addition, the `MacropaySolutions\Kernel\Support\ValidatedInput` instance may be iterated over and accessed like an array:
...
If you would like to add additional fields to the validated data, you may call the `merge` method:
    ...
    $validated = $request->safe()->merge(['name' => 'Surname Name']);
...
After calling the `errors` method on a `Validator` instance, you will receive an `MacropaySolutions\Kernel\Support\MessageBag` instance, which has a variety of convenient methods for working with error messages. The `$errors` variable that is automatically made available to all views is also an instance of the `MessageBag` class.
...
If you are validating an array form field, you may retrieve all the messages for each of the array elements using the `*` character:
...
Some of Framework's built-in validation rule error messages contain a `:value` placeholder that is replaced with the current value of the request attribute. However, you may occasionally need the `:value` portion of your validation message to be replaced with a custom representation of the value. For example, consider the following rule that specifies that a credit card number is required if the `payment_type` has a value of `cc`:
...
The field under validation must match the authenticated user's password. You may specify an [authentication guard](/authentication) using the rule's first parameter:
...
Since this rule requires several arguments, you may use the `Rule::dimensions` method to fluently construct the rule:
    ...
    use MacropaySolutions\Kernel\Validation\Rule;
...
The `filter` validator, which uses PHP's `filter_var` function, ships with Framework.
...
Instead of specifying the table name directly, you may specify the Obvious model which should be used to determine the table name:

    'user_id' => 'exists:App\Models\User,id'

If you would like to customize the query executed by the validation rule, you may use the `Rule` class to fluently define the rule. In this example, we'll also specify the validation rules as an array instead of using the `|` character to delimit them:

    use MacropaySolutions\Kernel\Database\Query\Builder;
    use MacropaySolutions\Kernel\Validation\Rule;
...
The field under validation must not be present _only if_ all the other specified fields are present.
...
The field under validation must be present _only if_ all the other specified fields are present.
...
The field under validation must be present and not empty _only if_ all the other specified fields are present and not empty.
...
The field under validation must be present and not empty _only when_ all the other specified fields are empty or not present.
...
> [!WARNING]  
> You should never pass any user controlled request input into the `ignore` method. Instead, you should only pass a system generated unique ID such as an auto-incrementing ID or UUID from an Obvious model instance. Otherwise, your application will be vulnerable to an SQL injection attack.

Instead of passing the model key's value to the `ignore` method, you may also pass the entire model instance. Framework will automatically extract the key from the model:
...
> [!NOTE]  
> The `$input` parameter passed to your closure will be an instance of `MacropaySolutions\Kernel\Support\Fluent` and may be used to access your input and files under validation.
...
Like the `$input` parameter passed to the closure, the `$item` parameter is an instance of `MacropaySolutions\Kernel\Support\Fluent` when the attribute data is an array; otherwise, it is a string.
...
In general, you should always specify the array keys that are allowed to be present within your array. Otherwise, the validator's `validate` and `validated` methods will return all the validated data, including the array and all of its keys, even if those keys were not validated by other nested array validation rules.
...
Framework provides a variety of validation rules that may be used to validate uploaded files, such as `mimes`, `image`, `min`, and `max`. While you are free to specify these rules individually when validating files, Framework also offers a fluent file validation rule builder that you may find convenient:
...
To ensure that passwords have an adequate level of complexity, you may use Framework's `Password` rule object:

    use MacropaySolutions\Kernel\Validation\Rules\Password;
...
Framework provides a variety of helpful validation rules; however, you may wish to specify some of your own. One method of registering custom validation rules is using rule objects. To generate a new rule object, you may use the `make:rule` Run command. Let's use this command to generate a rule that verifies a string is uppercase. Framework will place the new rule in the `app/Rules` directory. If this directory does not exist, Framework will create it when you execute the Run command to create your rule:

```shell
php run make:rule Uppercase
```
...
Once the rule has been created, we are ready to define its behavior. A rule object contains a single method: `validate`. This method receives the attribute name, its value, and a callback that should be invoked on failure with the validation error message:
    ...
    use Closure;
    use MacropaySolutions\Kernel\Contracts\Validation\ValidationRule;
...
Instead of providing a literal error message to the `$fail` closure, you may also provide a [translation string key](/localization) and instruct Framework to translate the error message:
...
If your custom validation rule class needs to access all the other data undergoing validation, your rule class may implement the `MacropaySolutions\Kernel\Contracts\Validation\DataAwareRule` interface. This interface requires your class to define a `setData` method. This method will automatically be invoked by Framework (before validation proceeds) with all the data under validation:
...
Or, if your validation rule requires access to the validator instance performing the validation, you may implement the `ValidatorAwareRule` interface:
    ...
    class Uppercase implements ValidationRule, ValidatorAwareRule
    {
         ...
         *
         * @var \MacropaySolutions\Kernel\Validation\Validator
         */
        protected $validator;
...
~~For a custom rule to run even when an attribute is empty, the rule must imply that the attribute is required.~~ To quickly generate a new implicit rule object, you may use the `make:rule` Run command with the `--implicit` option:

```shell
php run make:rule Uppercase --implicit
```
